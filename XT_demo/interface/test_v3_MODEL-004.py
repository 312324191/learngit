...
class testRegistAccount01(unittest.TestCase):
    ...
    def testNormal_MODEL_004(self):
        json_data = json.dumps(self.data)
        resp = requests.post(url=self.url, data=json_data,verify=False)
        resp_json = resp.json()
        self.assertEqual(resp_json.get('error').get('status'), "B-PF-036")
        self.assertEqual(resp_json.get('error').get('message'), u"特服产品包、增值产品包已订购，不能叠加订购，资费标识:PE_0140")
        
if __name__=='__main__':
    suite = unittest.TestSuite()
    suite.addTest(testRegistAccount01("testNormal_MODEL_004"))  # 按用例执行
