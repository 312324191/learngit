...
class testRegistAccount01(unittest.TestCase):
    def setUp(self):
        try:
            ...
            # 调用模板方法
            self.change_data_two, self.change_token_two, change_service_type_two, change_service_name_two = self.rt.modify_service_model(svc_number, imsi, template)
            # logging.debug('This is debug message self.data : %s' % self.data)
            # 拼装URL
            self.change_url_two = "https://%s:%s/OSN/vop/%s/%s/%s?token=%s" % (ip, host, change_service_type_two, change_service_name_two, Edition, self.change_token_two)
        except AttributeError:
            print u"请查看转企标识没有找到信息"

    def testNormal_MODEL_007(self):
        ...
        try:
            ...
            vp.close()
        sleep(60)
        json_data = json.dumps(self.change_data_two)
        resp = requests.post(url=self.change_url_two, data=json_data,verify=False)
        resp_json = resp.json()
        try:
            resp_json.get('data').get('message')
        except AttributeError:
        ...
        try:
            vp = Verifying_Point(self.data["timestamp"])
            ServiceOrder = vp.Service_Order(self.change_data_two["data"]["order_id"])
            # 竣工需要调用ServiceOrder返回参数用的bss_service_order_no 传入报文竣工方法中
            self.assertEqual(ServiceOrder[0][3], '0')
            self.assertEqual(ServiceOrder[0][4], '13')
...
if __name__=='__main__':
    ...
    suite.addTest(testRegistAccount01("testNormal_MODEL_007"))  # 按用例执行
